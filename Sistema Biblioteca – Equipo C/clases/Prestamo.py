...
class Prestamo:
    ...
    def __str__(self):
        return f"Prestamo: id = {self.id_prestamo}, Tipo ={self.tipo_producto}, Dias en prestamo ={self.dias_prestamo}, fecha de prestamo ={self.fecha_prestamo}, entrega estimada ={self.fecha_entrega_estimada}, activo ={self.activo}, id lector ={self.id_lector}"
    ...
    def registrar_prestamo(self, listacopias, listarticulos, listatesis, listalectores, listaprestamos):
        try:
            Id_prestamo = int(input("Ingrese el ID del prestamo: "))
            contador = 1
            for prestamo in listaprestamos:
                if (Id_prestamo == prestamo.get_id_prestamo()):
            ...
            if (contador > len(listalectores)):
                ...
                if (siono.upper() == 'S'):
                    ...
                    if (nuevoLector.crear(listalectores) == 1):
            ...
            if self.tipo_producto == "Libro":
                ...
                for copia in listacopias:
                    if copia.getID() == self.id_producto and copia.getID() == idlibro:
